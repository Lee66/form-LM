...
function creElm(o, pN, t) {
	var tmp = document.createElement(t || 'div'),
		p;
	for(p in o) {
		tmp[p] = o[p];
	}
...
function testFunction(o) {
	var val = o.value,
		len = Math.floor(strLength(val));
	return {
		stat: len < 50,
		msg: '已输入<strong>' + len + '</strong>个字符；',
		msgCss: (len < 50 ? 'TestClass_OK' : 'TestClass_Err'),
		inCss: null
	};
}
...
function ValidatorForm(f, Stat, Validator) {
	...
	if(Validator) {
		for(var i in this.Validator) {
			this.Validator[i] = Validator[i] !== undefined ? Validator[i] : this.Validator[i];
		}
	}
	if(Stat) {
		for(var i in this.Stat) {
			this.Stat[i] = Stat[i] !== undefined ? Stat[i] : this.Stat[i];
		}
	}
	this.Form = f;
	var fn = f.elements.length,
		i, j, _dataType;
	for(i = 0; i < fn; i++) {
		...
		for(j = 0; j < _dataType.length; j++) {
			if(this.Validator[_dataType[j].split('[')[0]]) {
				f.elements[i].onblur = (function(o) {
					return function() {
						vf.CheckData(o);
					}
				})(f.elements[i]);
	...
	this.submit = function(e) {
		var fn = vf.Form.elements.length,
			i, j, _Err = 0,
			_Focus, _Msg = '',
			t, _dataType;
		for(i = 0; i < fn; i++) {
...
ValidatorForm.prototype = {
	...
	CheckData: function(o) {
		var _dataType = o.getAttribute('dataType'),
			i, s = true,
			_Msg = '',
			_msgCss, _inCss, _mdt, min, max, _dt;
		_dataType = _dataType ? _dataType.split(' ') : [];
		for(i = 0; s && i < _dataType.length; i++) {
			if(this.Validator[_dataType[i].split('[')[0]]) {
				...
				//值对比
					...
					if(!this.Form[_mdt[2][0]] || this.Form[_mdt[2][0]].value == '') {
						return false;
					}
					...
					if(!s) {
						_Msg = (_mdt[1] == '=' || _mdt[1] == '!=') ? '必须和' + _mdt[2][1] + cc : '必须' + cc + _mdt[2][1];
					}
				}
				//日期格式验证
				else if(_dt == 'date') {
				...
				//其它类型验证
					...
					s = s && this.Validator[_dt][0].test(o.value);
					if(s) {
						s = s && this.Check_Num((_dt == 'number' ? o.value - 0 : strLength(o.value)), min, max);
					} else {
						_Msg = this.Validator[_dt][3];
					}
				}
				//生成提示文本
				if(_Msg == '' && !s) {
	...
	GetMsgBox: function(o) {
		...
		if(!MsgTag) {
			...
			while(MsgTag && MsgTag.nodeType != 1) {
				MsgTag = MsgTag.nextSibling;
			}
			if(!MsgTag || MsgTag.tagName.toUpperCase() != this.Stat.tag.toUpperCase() || !MsgTag.getAttribute('msgbox')) {
				...
				if(o.nextSibling.nextSibling) {
					o.parentNode.insertBefore(MsgTag, o.nextSibling);
				}
			}
		}
		return MsgTag;
	...
	Check_Compare: function(v, operator, v2, DataType) {
		//为空时自动判断格式
		if(!DataType) {
			if(this.Validator.number[0].test(v)) {
				DataType = 'number';
			} else if(this.Validator.date[0].test(v)) {
				DataType = 'date';
			} else {
				DataType = 'text'
			}
